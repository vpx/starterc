...
sed "${SED_INPLACE[@]}" "s/starterc/$GITHUB_REPOSITORY_NAME/g" .github/ISSUE_TEMPLATE/config.yml

find . -type d -name ".git" -prune -o -type f -print0 | while IFS= read -r -d '' file; do
    sed "${SED_INPLACE[@]}" "s/starterc/$PROJECT_NAME_LC/g" "$file"
    sed "${SED_INPLACE[@]}" "s/STARTERC/$PROJECT_NAME_UC/g" "$file"
done
